...
public class TownHelper {
    ...
    public CheckResult delete(TownModel obj) throws IntegrationException {
        CheckResult res = new CheckResult();
        res.setIsSuccess(true);

        if (obj.getId() == null)
            obj = getByCodeCityIdAdmTerritoryIdRegionIdTerDepIdBaseDepId(
                    obj.getCode(),
                    obj.getCityId(),
        ...
        townDAO.delete(obj.getId());

        if ( townDAO.getById(obj.getId()) != null) {
            res.setIsSuccess(false);
            res.setMessageTest("ERROR: STH_TOWN is not deleted.");
        }
