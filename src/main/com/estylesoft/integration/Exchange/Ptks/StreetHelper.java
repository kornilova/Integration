...
public class StreetHelper {
    ...
    public CheckResult delete(StreetModel obj) throws IntegrationException {
        CheckResult res = new CheckResult();
        res.setIsSuccess(true);

        if (obj.getId() == null)
            obj = getByCodeCityIdTownIdAdmTerIdRegionIdBaseDepId(
                    obj.getCode(),
                    obj.getCityId(),
                    obj.getTownId(),
        ...
        streetDAO.delete(obj.getId());

        if ( streetDAO.getById(obj.getId()) != null) {
            res.setIsSuccess(false);
            res.setMessageTest("ERROR: STH_STREET is not deleted.");
        }
