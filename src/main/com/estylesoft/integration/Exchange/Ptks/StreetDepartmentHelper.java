...
public class StreetDepartmentHelper {
    ...
    public CheckResult delete(StreetDepartmentModel obj) throws IntegrationException {
        CheckResult res = new CheckResult();
        res.setIsSuccess(true);

        if (obj.getId() == null)
            obj = getByStreetIdTerDepId(obj.getStreetId(), obj.getTerDepartmentPFRId());

        streetDepartmentDAO.delete(obj.getId());

        if ( streetDepartmentDAO.getById(obj.getId()) != null) {
            res.setIsSuccess(false);
            res.setMessageTest("ERROR: STH_STREET_TDP is not deleted.");
        }
