...
public class CityHelper {
    ...
    public CheckResult delete(CityModel obj) throws IntegrationException {
        CheckResult res = new CheckResult();
        res.setIsSuccess(true);

        if (obj.getId() == null)
            obj = getByCodeAdmTerrIdRegionIdTerDepIdBaseDepId(obj.getCode(),
                    obj.getAdmTerritoryId(),
                    obj.getRegionId(),
                    obj.getTerDepartmentPFRId(),
                    obj.getBaseDepartmentId());

        cityDAO.delete(obj.getId());

        if (cityDAO.getById(obj.getId()) != null) {
            res.setIsSuccess(false);
            res.setMessageTest("ERROR: STH_CITY is not deleted.");
        }
